...
by_cohort_ES_estimate_ATT <- function(ES_data,
  ...
  if(add_unit_fes == TRUE){
    ES_data[, unit_sample := .GRP, keyby = c(unit_var, "ref_onset_time", "catt_specific_sample")]
  } else{
    ES_data[, unit_sample := .GRP, keyby = c(onset_time_var, "ref_onset_time", "catt_specific_sample")]
  }
...
# Minor modification of ES_make_ipw_dt() -- all cases where we use .GRP + 'by' have been changed to .GRP + 'keyby'
#' @export
ES_make_ipw_dt2 <- function(did_dt,
                           unit_var,
                           cal_time_var,
                           discrete_covars = NULL,
                           cont_covars = NULL,
